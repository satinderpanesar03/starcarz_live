...
@section('title', 'Health Insurance Claim')
@section('content')
                                ...
                                <div class="card-header">
                                    <div class="row">
                                        <div class="col-12 col-sm-7">
                                            <h5 class="pt-2 pb-2">Manage Health Insurance Claim List</h5>
                                        </div>
                                        <div class="col-12 col-sm-5 d-flex justify-content-end align-items-center">
                                            <button class="btn btn-sm btn-danger px-3 py-1 mr-2" id="listing-filter-toggle">
                                                <i class="fa fa-filter"></i> Filter </button>
                                            <a href="{{route('admin.claim.health-insurance.create')}}" class="btn btn-sm btn-primary px-3 py-1">
                                                <i class="fa fa-plus"></i>Add Health Insurance Claim</a>

                                        </div>
                                    </div>
                                </div>
                                <form action="{{route('admin.claim.health-insurance.index')}}" method="get">
                                        ...
                                        <tbody>
                                            @foreach ($insurances as $value => $insurance)
                                                <tr>
                                                    ...
                                                    <td>{{$insurance->healthInsurance ? strtoupper($insurance->healthInsurance->policy_number) : ''}}</td>
                                                    <td>{{$insurance->healthInsurance->member_name ? strtoupper($insurance->healthInsurance->end_date) : ''}}</td>
                                                    <td>{{$insurance->healthInsurance ? strtoupper($insurance->healthInsurance->end_date) : ''}}</td>
                                                    <td>
                                                    <span style="white-space:nowrap;" class="">
                                                        <a href="{{route('admin.claim.health-insurance.view', $insurance->id)}}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{route('admin.claim.health-insurance.show', $insurance->id)}}" class="btn btn-success btn-sm" title="Edit">
