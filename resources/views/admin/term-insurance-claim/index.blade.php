...
@section('title', 'Term Insurance Claim')
@section('content')
                                ...
                                <div class="card-header">
                                    <div class="row">
                                        <div class="col-12 col-sm-7">
                                            <h5 class="pt-2 pb-2">Manage Term Insurance Claim List</h5>
                                        </div>
                                        <div class="col-12 col-sm-5 d-flex justify-content-end align-items-center">
                                            <button class="btn btn-sm btn-danger px-3 py-1 mr-2" id="listing-filter-toggle">
                                                <i class="fa fa-filter"></i> Filter </button>
                                            <a href="{{route('admin.claim.term-insurance.create')}}" class="btn btn-sm btn-primary px-3 py-1">
                                                <i class="fa fa-plus"></i>Add Term Insurance Claim</a>

                                        </div>
                                    </div>
                                </div>
                                <form action="{{route('admin.claim.term-insurance.index')}}" method="get">
                                    ...
                                    <table class="table table-striped table-bordered dom-jQuery-events">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$item->termInsurance ? strtoupper($item->termInsurance->policy_number) : ''}}</td>
                                                <td>{{$item->survyour_name ? ucfirst($item->survyour_name) : ''}}</td>
                                                <td><span style="white-space:nowrap;" class="">
                                                        <a href="{{route('admin.claim.term-insurance.view', $item->id)}}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{route('admin.claim.term-insurance.show', $item->id)}}" class="btn btn-success btn-sm" title="Edit">
