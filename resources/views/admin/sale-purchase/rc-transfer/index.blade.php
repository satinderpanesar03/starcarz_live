                            ...
                            <div class="card-content">
                                ...
                                <div class="card-body table-responsive">
                                            ...
                                            <div class="row col-sm-12 ml-2">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control executive-filter" id="executiveFilter" name="agent">
                                                            <option value="">Select Agent</option>
                                                            @foreach($agents as $id => $executive)
                                                            <option value="{{ $id }}" {{ request()->get('agent') == $id ? 'selected' : '' }}>{{ $executive }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                    ...
                                    <table class="table table-striped table-bordered dom-jQuery-events">
                                        ...
                                        @if($combinedData->isNotEmpty()) <!-- Check if data is available -->
                                        <tbody>
                                            @foreach($combinedData as $value => $sale)
                                            <tr>
                                                <td>{{ ++$value }}</td>
                                                <td>{{ ($sale->purchase) ? $sale->purchase->reg_number : $sale->vehicle_number }}</td>
                                                @if ($sale instanceof App\Models\AggregatorLoan)
                                                <td>{{ ($sale->year) ? $sale->year : (($sale->purchase) ? $sale->purchase->registration_year : '') }}</td>
                                                @else
                                                <td>{{ ($sale->registration_year) ? $sale->registration_year : (($sale->purchase) ? $sale->purchase->registration_year : '') }}</td>
                                                @endif
                                                ...
                                                <td class="text-truncate">
                                                    <span style="white-space:nowrap;" class="">
                                                        ...
                                                        <a href="{{ route('admin.rc-transfer.sale-view', ['id' => $sale->id]) }}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{ route('admin.rc-transfer.sale-show', ['id' => $sale->id]) }}" class="btn btn-success btn-sm" title="Edit">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                        @elseif ($sale instanceof App\Models\AggregatorLoan)
                                                        <a href="{{ route('admin.rc-transfer.aggregrator-view', $sale->id) }}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{ route('admin.rc-transfer.aggregrator-show', ['id' => $sale->id]) }}" class="btn btn-success btn-sm" title="Edit">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                        @elseif ($sale instanceof App\Models\CarLoan)
                                                        <a href="{{ route('admin.rc-transfer.car-view', ['id' => $sale->id]) }}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{ route('admin.rc-transfer.car-show', ['id' => $sale->id]) }}" class="btn btn-success btn-sm" title="Edit">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                        @endif
                                                    </span>
