                                ...
                                <div class="card-header">
                                    <div class="row">
                                        <div class="col-12 col-sm-7">
                                            <h5 class="pt-2 pb-2">Manage Banks List</h5>
                                        </div>
                                        <div class="col-12 col-sm-5 d-flex justify-content-end align-items-center">
                                            <button class="btn btn-sm btn-danger px-3 py-1 mr-2" id="listing-filter-toggle">
                                                <i class="fa fa-filter"></i> Filter </button>
                                            <a href="{{route('admin.master.bank.add')}}" class="btn btn-sm btn-primary px-3 py-1">
                                                <i class="fa fa-plus"></i> Add Bank </a>

                                        </div>
                                    </div>
                                </div>
                                <div class="card-body table-responsive">
                                    <form action="{{route('admin.master.bank.index')}}" method="get">
                                        <div class="row mb-2" id="listing-filter-data" data-select2-id="listing-filter-data" style="display:none;">
                                            <div class="row col-sm-12 ml-2">
                                                <div class="col-sm-3">
                                                    ...
                                                    <input class="form-control" placeholder="Bank Name" type="text" id="search" name="name" value="{{request()->query('name') ?? ''}}">
                                                </div>
                                                <div class="col-sm-3">
                                                    <span class="text">Enter Address</span>
                                                    <input class="form-control" placeholder="Address" type="text" id="search" name="address" value="{{request()->query('address') ?? ''}}">
                                                </div>
                                                <div class="col-sm-3">
                                    ...
                                    <table class="table table-striped table-bordered dom-jQuery-events">
                                        ...
                                        <tbody>
                                            @forelse($banks as $value => $bank)
                                            <tr>
                                                <td>{{++$value}}</td>
                                                <td>{{ucfirst($bank->name)}}</td>
                                                <td>{{ucfirst($bank->address)}}</td>
                                                <td>{{ucfirst($bank->city)}}</td>
                                                <td class="text-truncate">
                                                    <span style="white-space:nowrap;" class="">
                                                        <a href="{{route('admin.master.bank.view', $bank->id)}}" class="btn btn-primary btn-sm" title="View">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                        <a href="{{route('admin.master.bank.show', $bank->id)}}" class="btn btn-success btn-sm" title="Edit">
            ...
            $(document).ready(function() {
                ...
                $('.delete-color').click(function(event) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Are you sure you want to delete the bank "' + colorName + '"?',
                        icon: 'warning',
