                                                ...
                                                @foreach ($purchased as $value => $item)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @endphp
                                                            <span
                                                                class="badge {{ $statusClass }}">{{ $item->getStatusName($item->status) }}</span>
                                                        </td>
                                                        <td>{{ $item->owners }}</td>
                                                        <td>{{ date('d M, Y', strtotime($item->insurance_due_date)) }}</td>
                                                        @if (Auth::guard('admin')->user()->role_id == 1)
                                                            <td>{{ $item->purchaseOrder ? $item->purchaseOrder->price_p1 : '---' }}
                                                            </td>

                                                            <td>{{ $item->refurbishment_sum_total_amount }}</td>

                                                            <td>{{ $item->purchaseOrder ? $item->purchaseOrder->price_p1 + $item->refurbishment_sum_total_amount : '---' }}
                                                            </td>
                                                        @endif
                                                        ...
                                                        @if (Auth::guard('admin')->user()->role_id == 1)
                                                            <td><i style="cursor: pointer;" data-id="{{ $item->id }}"
                                                        ...
                                                        <td>
                                                            @if ($item->pending_image_status == 0)
                                                                <a class="btn btn-sm btn-danger"
                                                                    href="{{ route('admin.purchase.purchase.ready-sale-add-image', $item->id) }}">Pending
                                                                    Pics</a>
                                                            @else
                                                                <a class="btn btn-sm btn-success"
                                                                    href="{{ route('admin.purchase.purchase.ready-sale-add-image', $item->id) }}">IN
                                                                    STOCK</a>
                                                            @endif
                                                        </td>
            ...
            <form action="{{ route('admin.add.selling.price') }}" method="post">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="number" class="form-control" placeholder="Enter Selling Price" name="selling_price"
                            required>
                    </div>
